...
| 阶段 | 内容 | 状态 | 完成日期 | 备注 |
|------|------|------|----------|------|
| 一 | 核心业务 | ✅ 已完成 | 2026-07-07 | 用户/分类/商品/购物车/订单/支付 全部完成 |
| 二 | 安全与认证 | ✅ 已完成 | 2026-07-07 | JWT + Spring Security 过滤器链 |
| 三 | 性能与并发 ⭐ | ✅ 已完成 | 2026-07-07 | Redis 缓存 + 穿透/击穿/雪崩 + 分布式锁秒杀 |
| 四 | 异步与消息 | ✅ 已完成 | 2026-07-07 | RabbitMQ 死信队列 + 订单超时取消 + 库存预警 + 幂等 |
| 五 | 搜索与推荐 | ⬜ 未完成 | — | 选做（需部署 Elasticsearch） |
| 六 | 工程化 | ✅ 已完成 | 2026-07-08 | Knife4j API 文档 + JUnit 5 单元测试 |
| 七 | 加分项 | 🔄 已完成 | 2026-07-08 | WebSocket + 数据统计；微服务未做 |
...
| 3.4 | 秒杀功能（库存预扣减 + MQ 异步下单） | 🔄 已优化 | 同步扣库存(Redis锁) → 发MQ → 异步创建订单(削峰) |
